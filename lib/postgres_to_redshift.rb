...
class PostgresToRedshift
  ...
  def tables
    source_connection.exec("SELECT * FROM information_schema.tables WHERE table_schema = 'public' AND table_type in ('BASE TABLE') AND table_name NOT IN ('stripe_support_webhook_payloads', 'activities', 'pooled_quizzes_answers', 'user_sessions', 'stripe_support_card_fingerprints')").map do |table_attributes|
